 ...
 * <ul>
 *     <li>Prepends the URI path with API endpoint (using {@link RestTemplateUriPrefixingClientHttpRequestFactory}</li>
...
public abstract class SingleEndpointGoodDataRestProvider implements GoodDataRestProvider {
    ...
    protected RestTemplate createRestTemplate(final GoodDataEndpoint endpoint, final GoodDataSettings settings, final HttpClient httpClient) {
        ...
        final ClientHttpRequestFactory httpClientFactory = new HttpClient5ComponentsClientHttpRequestFactory(httpClient);
        final ClientHttpRequestFactory factory = new RestTemplateUriPrefixingClientHttpRequestFactory(
                httpClientFactory,
                java.net.URI.create(endpoint.toUri())
        );
    ...
    protected HttpClientBuilder createHttpClientBuilder(final GoodDataSettings settings) {
        // Create connection manager using HttpClient 5.x builder pattern
        ...
        // Create request config using HttpClient 5.x APIs
